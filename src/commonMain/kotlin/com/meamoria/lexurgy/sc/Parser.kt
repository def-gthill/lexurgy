...
object LscWalker : LscBaseVisitor<LscWalker.ParseNode>() {
    ...
    private data class RuleContext(
        ...
        val precedingElement: RuleElement?,
        val followingElement: RuleElement?,
    ) {
        companion object {
            fun aloneInMain(): RuleContext =
                RuleContext(RuleSection.MAIN, null, null)

            fun rightBeforeAnchor(): RuleContext =
                RuleContext(RuleSection.ENVIRON, null, Anchor)

            fun rightAfterAnchor(): RuleContext =
                RuleContext(RuleSection.ENVIRON, Anchor, null)

            fun nowhere(): RuleContext =
                RuleContext(RuleSection.NOWHERE, null, null)
        }
    }
    ...
    /**
     * Represents the anchor (the underscore) in a rule environment.
     * This will never be generated by the parser, but may show up
     * as a preceding or following element in a RuleContext.
     */
    private object Anchor : RuleElement {
        override val text: String = "_"

        override val publicName: String = "an underscore"
    ...
    private object WordBoundaryElement : BaseParseNode("$"), ChameleonRuleElement {
        ...
        override fun <T : Segment<T>> link(context: RuleContext): Matcher<T> =
            when {
                context.precedingElement == null -> WordStartMatcher()
                context.followingElement == null -> WordEndMatcher()
    ...
    private class SequenceElement(
        ...
        private fun <T : Segment<T>> link(
            ...
            try {
                SequenceMatcher(
                    (listOf(context.precedingElement) + elements +
                            listOf(context.followingElement)).windowed(3) {
                        val (preceding, current, following) = it
                        current!!.linker(context.copy(precedingElement = preceding, followingElement = following))
                    }
                )
            } catch (e: LscBadSequence) {
    ...
    private class RepeaterElement(
        ...
        private fun checkContext(context: RuleContext) {
            if (context.section == RuleSection.ENVIRON &&
                (context.precedingElement == null || context.followingElement == null)
            ) {
                throw LscPeripheralRepeater(text)
            }
        }

        private fun removeAnchor(element: RuleElement?): RuleElement? =
            element?.takeIf { it !is Anchor }
    }
